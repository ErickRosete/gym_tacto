...
class SawyerGraspEnv(gym.Env):
    ...
    def reset_initial_positions(self):
        ...
        self.robot.reset()
        self.grasp_obj.set_base_pose(
            self.get_grasp_obj_initial_position(), self.grasp_obj.init_base_orientation)
        if self.show_gui:
            p.configureDebugVisualizer(p.COV_ENABLE_RENDERING, 1)
    ...
    def step(self, action):
        ...
        info = {"grasp_obj_position": self.get_grasp_obj_position(), "success": success} 
        return observation, reward, done, info

    def get_grasp_obj_initial_position(self):
        if self.random_obj_position:
            obj_position = [np.random.uniform(0.40, 0.60), np.random.uniform(-0.1, 0.1), 0]
        else:
            obj_position = [0.5, 0, 0]
        return obj_position

    def load_grasp_objects(self):
        obj_position = self.get_grasp_obj_initial_position()
        grasp_obj_urdf_path = pkg_path("envs/" + self.cfg.objects.square_prism.urdf_path)
        grasp_obj_cfg = {"urdf_path": grasp_obj_urdf_path, "base_position": obj_position,
                    "base_orientation": p.getQuaternionFromEuler((0, 0, 0))}
        self.grasp_obj = px.Body(**grasp_obj_cfg, physics_client=self.physics_client)
        self.digits.add_body(self.grasp_obj)
    ...
    def get_grasp_obj_position(self):
        return np.array(self.grasp_obj.get_base_pose()[0])
...
class TransformObservation(ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        if self.relative:
            state_target = self.env.get_grasp_obj_position()
...
def make_env_sawyer_grasp_v0(cfg):
    env = gym.make('gym_tacto:sawyer-grasp-v0', cfg=cfg)
