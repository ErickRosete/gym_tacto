...
class PDController():
    ...
    def init_target(self):
        target_position = self.env.get_grasp_obj_position()
        gripper_width = self.env.get_gripper_width()
        self.target = [target_position[0], target_position[1], target_position[2] + 0.15, gripper_width]    
    ...
    def change_target(self, dx, dy, dz, dgw):
        if abs(dx) < self.error and abs(dy) < self.error and  abs(dz) < self.error and abs(dgw) < self.error:
            ...
            gripper_width = self.env.get_gripper_width()
            if self.state == 1:
                self.target = [target_position[0], target_position[1], target_position[2] + 0.015, gripper_width]
            elif self.state == 2:
                self.target = [target_position[0], target_position[1], target_position[2] + 0.015, 0.075]   
